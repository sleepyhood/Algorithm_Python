...
while q:
    ...
    for i in range(4):
        ...
        if miro[nr][nc] == 1 and broken == 0 and not visited[nr][nc][1] :   # 벽을 만나도, 아직 마법을 사용할 수 있다면
            # 벽이 있는 경우 and 이전에 벽을 부순적이 없음 and 벽을 파괴할 다음 지역이 방문 안된곳
            q.append([nr, nc, 1])
            # 벽을 파괴한 것으로 간주
